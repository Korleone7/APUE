 ...
 *  这是因为：
 *  	父进程执行ls命令，将输出结果给通过管道传递给子进程后去执行wc命令时，
 *  	父进程若先于子进程打印wc运行结果之前，被shell使用wait函数成功回收，
 *  	shell就会先于子进程打印wc运行结果之前打印$提示符。
 *  解决方法：
 *  	让子进程执行ls,父进程执行wc命令。或者在兄弟进程间完成。
 */
